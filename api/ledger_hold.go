...
func PostLedgerHold(c *gin.Context) {
	...
	txErr := database.DB.Transaction(func(tx *gorm.DB) error {
		// Анти-дубль happy-path: явная проверка даёт чистый 409 без удара в constraint.
		// Гонку двух операторов ловит partial-unique → ниже мапим 23505 в 409 (Codex #5).
		var cnt int64
		if e := tx.Table("public.billing_holds").
			Where("contract_id = ? AND admin_account_id = ? AND company_id = ? AND active = ? AND deleted_at IS NULL",
				req.ContractID, adminAccountID, contract.CompanyID, true).Count(&cnt).Error; e != nil {
			return e
		}
		if cnt > 0 {
		...
		// Снять активную debt-приостановку и вернуть договор в прежний статус.
		var susp models.BillingSuspension
		if tx.Table("public.billing_suspensions").
			Where("contract_id = ? AND admin_account_id = ? AND company_id = ? AND reason = ? AND active = ? AND deleted_at IS NULL",
				req.ContractID, adminAccountID, contract.CompanyID, "billing_debt", true).First(&susp).Error == nil {
			...
			// Restore PreviousStatus (sweep блокирует только из 'active', но fail-safe →
			// fallback 'active'). UPDATE условный по 'suspended' — не топчем ручной cancel/expired.
			restore := susp.PreviousStatus
			if restore == "" {
				restore = "active"
			}
			if e := tx.Table(contractsTable).Where("id = ? AND status = ?", req.ContractID, "suspended").
				Update("status", restore).Error; e != nil {
				return e
			}
		}
		return nil
...
func GetLedgerHolds(c *gin.Context) {
	...
	database.DB.Where("contract_id = ? AND admin_account_id = ? AND deleted_at IS NULL", uint(contractID), adminAccountID).
		Order("active DESC, created_at DESC").Limit(200).Find(&holds)
