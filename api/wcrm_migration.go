...
func contractPeriodFromAppendices(ct wcrmContract, startOverrides map[int64]time.Time, autoStart time.Time) (time.Time, *time.Time, bool) {
	...
	for _, ap := range ct.Appendices {
		...
		if !found || effSt.Before(minStart) {
			minStart = effSt
		}
		found = true
...
type wcrmApproveRequest struct {
	...
	// Override двигает ТОЛЬКО StartDate подписки; конец (end) считается от оригинального старта.
	AppendixStartOverrides map[string]string `json:"appendix_start_overrides"`
}
...
func PostWcrmMigrationApprove(c *gin.Context) {
	...
	for k, v := range req.AppendixStartOverrides {
		...
		if t, terr := time.Parse("2006-01-02", strings.TrimSpace(v)); terr == nil {
			startOverrides[id] = t.UTC()
		}
	}
		...
		if rec.Balance != 0 {
			...
			if primaryContractID > 0 {
				...
				tx.Table(fmt.Sprintf("tenant_%d.contracts", wcrmTargetTenantID)).
					Select("counterparty_id, currency").Where("id = ?", primaryContractID).Scan(&prim)
				primaryCP := prim.CounterpartyID
				primaryCcy := contractCurrency(prim.Currency)
				...
				if existsCount == 0 {
					...
					if err := tx.Model(&models.LedgerEntry{}).
						...
						Updates(map[string]interface{}{"amount": amount, "counterparty_id": primaryCP, "currency": primaryCcy}).Error; err != nil {
